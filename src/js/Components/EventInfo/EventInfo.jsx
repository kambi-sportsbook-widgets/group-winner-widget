...
const EventInfo = ({ homeName, homeScore, awayName, awayScore, eventName }) => {
   ...
   const formattedDate = date.getDate() + ' ' + t('month' + [date.getMonth()]) + ' ' + date.getFullYear();
   return (
      <div>
         <EventInfoContainer>
         ...
         { <div className={styles.eventName + ' KambiWidget-card-support-text-color'}>{formattedDate}</div> }
      </div>)
};
